...
export class ArticlesService {
  ...
  getArticles(
    ...
    return this.http.get<PagedResult<ArticleListDto>>(this.apiUrl, { params }).pipe(
  ...
  incrementViewCount(id: number): Observable<any> {
    return this.http.put(`${this.apiUrl}/${id}/view`, {}).pipe(
      catchError(this.handleError)
    );
  }
  ...
  getAllArticlesForAdmin(
    ...
    return this.http.get<PagedResult<ArticleListDto>>(`${this.apiUrl}/admin`, { params }).pipe(
      catchError(this.handleError)
    );
  }

  getArticleByIdForAdmin(id: number): Observable<ArticleDto> {
    return this.http.get<ArticleDto>(`${this.apiUrl}/admin/${id}`).pipe(
      catchError(this.handleError)
    );
  }

  createArticle(article: ArticleCreateUpdateDto): Observable<ArticleDto> {
    return this.http.post<ArticleDto>(this.apiUrl, article).pipe(
      catchError(this.handleError)
    );
  }

  updateArticle(id: number, article: ArticleCreateUpdateDto): Observable<ArticleDto> {
    return this.http.put<ArticleDto>(`${this.apiUrl}/${id}`, article).pipe(
      catchError(this.handleError)
    );
  }
